...
PropBool(Player, StaticPrepassQuickRefresh, "Quick static prerender refresh"s, "Render the static prerender in a single pass that matches the dynamic frames (no supersampling, mipmaps kept) instead of the 128-sample bake. Needed when a head-tracking plugin hands the prepass back and forth mid-game: cheap to re-bake, and no visible change when switching"s, false);
