...
PropBool(Player, StaticPrepassQuickRefresh, "Quick static prerender refresh"s, "After the full startup prerender, re-bake the static parts in a single pass (no supersampling, mipmaps kept) whenever a plugin hands static prerendering back mid-game — e.g. head tracking that holds the eye still. Cheap, and the re-baked statics match the dynamic frames"s, false);
